...
class LiveFuelModel:
    ...
    def is_acceptable_granule(self, granule):
        return self.hv_for_modis_granule(granule) in self.used_granules()
    ...
    def date_for_modis_granule(self, granule) -> dt.datetime():
    ...
    def retrieve_earth_observation_data(url):
        ...
        if not self.self.storage_engine.swift_check_lfmc(hdf5_name):
            # No LFMC Product for this granule
            if not self.self.storage_engine.swift_check_modis(file_name):
    ...
    def convert_modis_granule_file_to_lfmc(fobj) -> xr.DataArray:
